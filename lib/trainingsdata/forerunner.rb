...
class Forerunner < Base
  attr_accessor :xml, :rounds, :time_total,
                ...
                :lap_single_map, :lap_single_heartrate, :lap_single_height, :source_doc , :diff_time_in_move
  ...
  def initialize
    ...
    @speed_max            = 0
    @time_total           = 0
    @time_in_move         = Time.parse('2000-07-14 15:10:26')
    @diff_time_in_move    = 0
    @distance_total       = 0
    @calories             = 0
    @start_time           = 0
      ...
      if node.name == 'Lap'
        ...
        node.children.each do |main_sub_node|
          ...
          if main_sub_node.name == 'AverageHeartRateBpm'
            @rounds[round] ||= {}
            pp  main_sub_node.children.text
          ...
          if sub_node.name == 'Trackpoint'
            ...
            sub_node.children.each do |working_node|
              if working_node.name == 'Time'
                ...
                if(trackpoint == 1)
                  @time_in_move = Time.parse(working_node.inner_text.to_s)
                else
                  @diff_time_in_move  = TimeDifference.between(Time.parse(working_node.inner_text.to_s), @time_in_move).in_seconds + @diff_time_in_move
                  @time_in_move       = Time.parse(working_node.inner_text.to_s)
                end
