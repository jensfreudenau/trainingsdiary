 ...
 * @version 1.10
 */
var GarminDeviceControlJens = Class.create();

GarminDeviceControlJens.prototype = {

    initialize: function(statusDiv, mapId, keysArray, authToken) {
        this.auth = authToken;
...
//                        this.mc.map.clearMarkers();
            ...
            if (this.factory != null) {
                ...
                switch(this.fileTypeRead) {
                    ...
                    case Garmin.DeviceControl.FILE_TYPES.crsDetail:
                        ...
                        this.allActivities.push(json.controller['gpsDataString']);
                        // Not finished with the activity queue
                        if( this.activityQueue.length > 0) {
                            this._readSelectedActivities(json);
                ...
                if(this.allActivities.length > 0){
                    try {
                        for (var i = 0; i < this.allActivities.length; i++) {
                            var active =this.allActivities[i];
                            jQuery('#ajaxcall').show();
                            jQuery.ajax({ url: '/trainings/saveWorkouts',
                                type: 'POST',
                                dataType: "html",
                                data: {
                                    training: Object.toJSON(active)  ,
                                    authenticity_token:  this.auth
                                },
                                success: function(result) {
                                    success = true;
                                    jQuery("#result").append(result);
                                    jQuery('#readBox').slideUp("slow");
                                    jQuery('#ajaxcall').slideUp("slow");
                                },
                                error: function(){
                                    console.log("Ajax answer is wrong!");
                                }
                            });
                        }
                    }
     ...
     */
    _readSelectedActivities: function(json) {
    ...
    _addToActivityListing: function(index, activity) {
        ...
        var sumVal          = activity.getSummaryValue(Garmin.Activity.SUMMARY_KEYS.startTime).getValue();
        var checkbox        = document.createElement("input");
        checkbox.checked    = 'checked';
        try {
            new Ajax.Request('/trainings/getWorkoutByStartTime', {
                method:'get',
                parameters: {start_time: sumVal.getXsdString(), limit: 1},
                asynchronous: false,
    ...
    _listActivities: function(activities) {
        ...
        if(numOfTracks > 0) {
            for (var k = 0; k < this.activities.length; k++) {
                var activity = this.activities[parseInt(k)];
            }
