...
//         	this.clearActivityQueue();  // Clearing does not work for whatever reason... timing/async
	...
	_addToActivityTable: function(index, entry) {
        ...
        var rowbody =this.activityTable.appendChild(tBody);
		var row = rowbody.insertRow(rowbody.rows.length); // append a new row to the table
		// Color odd rows
//		if( (index+2) % 2 != 0) {
...
//		}
        ...
        if( entry.isNew == 'false') {
            ...
            statusCell.innerHTML ='imported';
        }
        else {
            statusCell.className = 'activityStatusCell new';
	...
	_addToActivityTableHeader: function(directory) {
	    ...
	    var tableIndex = 0;
        var header=this.activityTable.createTHead();
        var row=header.insertRow(0);



	    //var row = this.activityTable.insertRow(0); // append a new row to the table
        row.className = "header_cell";
		var selectCell = row.insertCell(tableIndex++);
		...
		if( this.options.uploadMaximum < 1) {
    		...
    		this.checkAllBox.checked = false; // entries are "new" by default
...
Garmin.DeviceDisplayDefaultOptions =
	...
	*/
	 ...
	 * @type String 
	 */
	actionButtonClassName:		"actionButton button",
	 ...
	 * @type String  */
	statusCellProcessingImg: '<img src="/assets/ajax-loader.gif" width="15" height="15" />',
	...
	load: function() {
		...
		.each(
			function(s) {
				...
				var dependencies =   'device/GarminDeviceControl' +
									',device/GarminDevicePlugin' +
									',device/GarminGpsDataStructures' +
									',device/GoogleMapController' +
									',device/GarminDevice' +
									',device/GarminPluginUtils' +
									',api/GarminRemoteTransfer' +
									',util/Util-XmlConverter' +
									',util/Util-Broadcaster' +
									',util/Util-DateTimeFormat' +
									',util/Util-BrowserDetect' +
									',util/Util-PluginDetect' +
									',device/GarminObjectGenerator' +
									',activity/GarminMeasurement' +
									',activity/GarminSample' +
									',activity/GarminSeries' +
									',activity/GarminActivity' +
									',activity/GarminActivityDirectory' +
									',activity/GarminActivityFilter' +
									',activity/GarminActivityMatcher' +
									',activity/TcxActivityFactory' +
									',activity/GpxActivityFactory'+
									',directory/GarminDirectoryFactory'+
									',directory/GarminDirectoryUtils'+
									',directory/GarminFile';
